...
def parse_args():
    ...
    parser.add_argument(
        "--labeldir",
        type=str,
        default="./data/east/test_region/dataset.csv",
        help="Label dir",
    )
...
def train(opt):
    ...
    log_file = open(os.path.join(savepath, "log.txt"), "w")  # log train imformation
    ...
    val_loader = DataLoader(
        val_dataset, batch_size=128, num_workers=6, shuffle=False, pin_memory=True, prefetch_factor=2
    )
    ...
    if opt.scheduler == "ReduceLROnPlateau":
        ...
        scheduler = lr_scheduler.CosineAnnealingLR(
            optimizer, T_max=30, eta_min=0.00001, last_epoch=-1, verbose=False
        )
