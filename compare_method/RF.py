...
def train_RF(X_train, X_test, label_trian_list, label_test_list, savepath1, num_tree=1500):
    y_train = data_load(label_trian_list).ravel()
    y_test = data_load(label_test_list).ravel()
    sp = os.path.join(savepath1, str(num_tree))
    # 创建随机森林回归模型
    model = RandomForestRegressor(
        n_estimators=num_tree,   # 树的数量
        max_features=7,          # 最大特征数为7
        min_samples_leaf=1,      # 叶节点的最小样本数为1
        min_samples_split=5,     # 分裂内部节点所需的最小样本数为5
        random_state=3, 
    )
    ...
    # 画图
    min, max = draw_points(predictions_train, y_train, sp, label + "_train")
    draw_points(predictions_test, y_test, sp, label + "_test", maxv=max, minv=min)

    # 记录结果
    with open(f"{sp}//{label}_results.txt", "w", encoding="utf-8") as f:
        f.write(
            f"""Train R: {r_train:.4f} 
            MAE: {mae_train:.4f} 
            RMSE: {rmse_train:.4f} 
            R2: {r2_train:.4f} 
            Test R: {r_test:.4f} 
...
if __name__ == "__main__":
    ...
    savepath = "./compare_method/RF/result"
    ...
    for num_tree in num_trees:
        for label in tqdm(labels, desc=f"Running {num_tree}"):
            ...
            train_RF(data_train, data_test, label_train_list, label_test_list, label, num_tree)
